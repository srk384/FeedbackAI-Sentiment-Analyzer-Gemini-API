...
const Home = () => {
  ...
  // Wake backend when the site is opened
  useEffect(() => {
    const wake = async () => {
      try {
        await fetch((import.meta.env.VITE_API_BASE_URL || "") + "/wake", {
          method: "GET",
          cache: "no-store",
        });
      } catch (e) {
        console.debug("Wake call failed (ignored).");
      }
    };
    wake();
  }, []);
      ...
      <main className="flex flex-col gap-10">
        ...
        <section>
          <div className="flex flex-col items-end p-2 mx-auto sm:w-3xl">
            {Message && (
              <div className="sm:max-w-xl bg-gray-700 p-4 rounded-tr-sm rounded-2xl shadow-2xl transition text-md mb-10">
                {Message}
              </div>
            )}
            ...
            {Analysis && (
              <div>
                <div className="sm:w-3xl p-4 transition text-md mx-auto">
                ...
                <div className="sm:w-3xl p-4 transition text-md mx-auto">
                  ...
                  {Analysis.suggested_responses
                    ? Analysis.suggested_responses.map((item, index) => (
                        <li key={index} className="leading-8">
